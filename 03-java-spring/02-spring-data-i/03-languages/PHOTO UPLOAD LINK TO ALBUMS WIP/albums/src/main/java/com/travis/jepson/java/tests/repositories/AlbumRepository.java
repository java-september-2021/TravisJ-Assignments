...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	...
	@Query(value= "SELECT id, album_name, COUNT(album_likes.user_id) AS liked_users FROM albums LEFT JOIN album_likes " // MAKE SURE SPACE IS HERE
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	...
	@Query(value= "SELECT id, album_name, COUNT(album_likes.user_id) AS liked_users FROM albums LEFT JOIN album_likes " // MAKE SURE SPACE IS HERE
			+ "on albums.id = album_likes.album_id GROUP BY id, album_name ORDER BY liked_users ASC;", nativeQuery=true)
	List<Object[]> likesTotalAscendingFilter();
